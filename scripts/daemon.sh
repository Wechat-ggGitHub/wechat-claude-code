...
# Ensure systemd user session environment is set
linux_ensure_user_session() {
  # Set XDG_RUNTIME_DIR if not defined
  if [ -z "${XDG_RUNTIME_DIR:-}" ]; then
    export XDG_RUNTIME_DIR="/run/user/$(id -u)"
    mkdir -p "$XDG_RUNTIME_DIR" 2>/dev/null || true
  fi

  # Set DBUS_SESSION_BUS_ADDRESS if not defined
...
linux_node_bin() {
  ...
  if [ -z "$node_bin" ]; then
    # Try common nvm/fnm/volta locations
...
# Direct (non-systemd) start for environments without systemd user session
linux_direct_start() {
